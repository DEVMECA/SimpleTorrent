...
import com.devmeca.simpletorrent.BuildConfig;
import com.devmeca.simpletorrent.R;
import com.devmeca.simpletorrent.core.InputFilterRange;
import com.devmeca.simpletorrent.core.exception.NormalizeUrlException;
...
import com.devmeca.simpletorrent.core.model.data.TorrentStateCode;
import com.devmeca.simpletorrent.core.model.data.entity.Torrent;
import com.devmeca.simpletorrent.core.urlnormalizer.NormalizeUrl;
import com.devmeca.simpletorrent.core.utils.Utils;
import com.devmeca.simpletorrent.databinding.FragmentDetailTorrentBinding;
import com.devmeca.simpletorrent.ui.BaseAlertDialog;
import com.devmeca.simpletorrent.ui.FragmentCallback;
import com.devmeca.simpletorrent.ui.errorreport.ErrorReportDialog;
...
public class DetailTorrentFragment extends Fragment
{
    ...
    private static final int PICK_TORRENT_FILE_CHOOSE_REQUEST = 1;
    ...
    private void prepareOptionsMenu(Menu menu)
    {
        ...
        MenuItem runFileTorrentMenu = menu.findItem(R.id.run_file_torrent_menu);
        if (torrent != null && torrent.downloadingMetadata) {
            saveTorrentFile.setVisible(false);
            runFileTorrentMenu.setVisible(false);
        }
        else {
            saveTorrentFile.setVisible(true);
            runFileTorrentMenu.setVisible(true);
        }
    }
    ...
    private void runFileTorrent() {
        ...
        // move to file tab
        binding.fragmentViewpager.setCurrentItem(2);

        File downloadFile = new File(viewModel.info.getDirName() + "/" + viewModel.info.getTorrent().name);
        Uri uri = FileProvider.getUriForFile(getContext(), BuildConfig.APPLICATION_ID + ".provider", downloadFile);

        startActivity(viewModel.makeOpenFileIntent(viewModel.info.getTorrent().name, uri));
    ...
    private void shareMagnet(boolean includePriorities)
    {
        ...
        if (magnet != null) {
            ...
            sharingIntent.putExtra(Intent.EXTRA_SUBJECT, "magnet");
            sharingIntent.putExtra(Intent.EXTRA_TEXT, magnet);
    ...
    {
        if (requestCode != SAVE_TORRENT_FILE_CHOOSE_REQUEST
                || resultCode != Activity.RESULT_OK
                || requestCode != PICK_TORRENT_FILE_CHOOSE_REQUEST)
            return;
        ...
        try {
            ...
            if(PICK_TORRENT_FILE_CHOOSE_REQUEST == requestCode){
